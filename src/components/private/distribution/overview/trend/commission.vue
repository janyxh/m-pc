  ...
  <div class="item">
    <h3 class="title">佣金趋势
      <el-popover
        placement="top"
        trigger="hover"
        content="订单金额:同上，不包含已关闭订单、全额退款订单和已退款订单部分的已退款部分的金额，不包含运费。
佣金:已结佣、待结算和可提现三种状态中佣金综合。
销售佣金:分配给该订单的二级分销员的佣金。
邀请奖励佣金:分配给该订单的一级分销员的佣金。"
        width="200"
      >
    ...
    <div class="chart-wrap">
      <div class="charts" id="chartCommission" :style=" 'width:'+ chartWidth"></div>
    </div>
  </div>
</template>
...
import { getOption } from "@/mixins/chartOption/distribution/commission.js";
export default {
  props: ["isResize"],
  data() {
    return {
  ...
  methods: {
    ...
    load() {
      chartsDraw(
        document.querySelector("#chartCommission"),
